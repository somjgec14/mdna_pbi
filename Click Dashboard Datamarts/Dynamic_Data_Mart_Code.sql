...
FROM (
    SELECT DISTINCT
        ...
        -- Extract schema name from SQL text
        TRIM(REPLACE(
    ...
    INNER JOIN (
        ...
        ON v_hist.user_id = v_users.user_id
    LEFT JOIN (    
...
-- Keep only schemas that are in the allowed list
JOIN mard_allowed a
  ON q.Parsed_Schema = a.schema_name

# Script to assign Power BI workspaces to a specific capacity

# Capacity details
$capacityId = "40EF7A5E-1383-4947-A015-D6C46AFB919B"

# Array of workspace IDs to assign to the capacity
$workspaceIds = @(
...
# Log file
$logFile = "PowerBI_Capacity_Assignment_Log_$(Get-Date -Format 'yyyyMMdd_HHmmss').txt"
"Starting capacity assignment process at $(Get-Date)" | Out-File -FilePath $logFile

# Check for Power BI PowerShell module
...
foreach ($workspaceId in $workspaceIds) {
    try {
        ...
        $message = "Processing workspace: $workspaceId - Assigning to capacity: $capacityId"
        ...
        # The API endpoint is: groups/{workspaceId}/AssignToCapacity
        $response = Invoke-PowerBIRestMethod -Url "groups/$workspaceId/AssignToCapacity" -Method Post -Body $body
        
        # Log success
        $successMessage = "SUCCESS: Assigned workspace $workspaceId to capacity $capacityId"
        ...
        # Log any errors
        $errorMessage = "ERROR: Failed to assign workspace $workspaceId to capacity. Error: $($_.Exception.Message)"
    ...
    $currentCount = $successCount + $failureCount
    $percentComplete = [math]::Round(($currentCount / $totalWorkspaces) * 100, 2)
    Write-Progress -Activity "Assigning Workspaces to Capacity" -Status "$currentCount of $totalWorkspaces Complete ($percentComplete%)" -PercentComplete $percentComplete
...
Successful Assignments: $successCount
Failed Assignments: $failureCount
===================
...
"Completed capacity assignment process at $(Get-Date)" | Out-File -FilePath $logFile -Append
